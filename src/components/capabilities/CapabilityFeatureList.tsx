...
/* صفحهٔ هر قابلیت — grouped, scannable feature list.
   Naming only; no descriptions until phase 2. */
    ...
    return (
      <Reveal>
        <div className="rounded-m border-2 border-dashed border-line bg-surface p-8 text-center">
          ...
          <h2 className="mt-5 font-display text-[19px] font-extrabold text-ink">این قابلیت در فاز بعدی فعال می‌شود</h2>
          <p className="mx-auto mt-2 max-w-md text-[13.5px] leading-7 text-ink2">
            «{category.title}» در نقشهٔ محصول ثبت شده است؛ فهرست قابلیت‌های آن پس از انتشار اضافه خواهد شد.
          </p>
        </div>
      </Reveal>
    );
      ...
      <Reveal>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-[12.5px] font-semibold text-ink3 fa-num">
            {featureCount(category).toLocaleString("fa-IR")} قابلیت در {category.featureGroups.length.toLocaleString("fa-IR")} گروه
          </span>
        </div>
      </Reveal>

      <div className="mt-6 space-y-6">
        {category.featureGroups.map((group, gi) => (
