...
const FIELD =
  "w-full rounded-s border bg-bg px-4 py-3 text-[14px] text-ink outline-none transition-colors placeholder:text-ink3 focus:border-primary/60 focus:ring-2 focus:ring-primary/15";
...
function Submit({ label }: { label: string }) {
  ...
  return (
    <button
      ...
      disabled={pending}
      className={cn(
        "inline-flex h-12 w-full items-center justify-center gap-2 rounded-s bg-primary px-6 text-[14.5px] font-bold text-on-primary transition-all",
...
export function LeadForm({
  ...
  if (state?.ok) {
    return (
      <div className="rounded-m border border-accent/30 bg-accent-soft/40 p-8 text-center">
  ...
  return (
    <form action={action} noValidate className="space-y-5">
      ...
      {state?.ok === false && state.message && (
        <p role="alert" className="rounded-s border border-err/30 bg-err/5 px-4 py-3 text-[13.5px] font-semibold text-err">
          {state.message}
        </p>
      )}
